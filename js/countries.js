...
const change_country_button = document.querySelector(".change-country")
const clear_search_button = document.querySelector(".close")
const input = document.getElementById('search-input')

function createCountryList(){
    const num_countries = country_list.length;
    const num_of_ul_lists = 3;
    var ul_list_num, i=0;

    country_list.forEach((country, index) =>{
        if(index % Math.ceil(num_countries / num_of_ul_lists) == 0){
            ul_list_num = `list-${i}`;
            country_list_element.innerHTML += `<ul id= '${ul_list_num}'></ul>`; 
            i++;
        }
        document.getElementById(`${ul_list_num}`).innerHTML += `
...
change_country_button.addEventListener("click", function(){
    if (input){
        input.value = "";
    }
    resetCountryList();
...
if(input){
    input.addEventListener("input", function(){
        ...
        var country_val = input.value.toLowerCase();
        country_list.forEach(country => {
            if (country.name.toLowerCase().startsWith(country_val)){
                document.getElementById(country.name).classList.remove("hide");
            } else{
